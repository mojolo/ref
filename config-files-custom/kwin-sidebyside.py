...
def center_window():
    ...
    if w < zoom_2w:
        if win_class[1] == "kate":
            subprocess.run(["wmctrl", "-i", "-r", f'{win_id}', f'-e 0,{zoom_2x},{zoom_y + 33},{zoom_2w},{zoom_h}'])
        elif win_class[1] == "ktty":
            subprocess.run(["wmctrl", "-i", "-r", f'{win_id}', f'-e 0,{zoom_2x},{zoom_y},{zoom_2w},{zoom_h + 34}'])
        elif win_class[1] == "FocusProxy" or win_class[1] == "Code":
            subprocess.run(["wmctrl", "-i", "-r", f'{win_id}', f'-e 0,{zoom_2x},{zoom_y},{zoom_2w},{zoom_h}'])
