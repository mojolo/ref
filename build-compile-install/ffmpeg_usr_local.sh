...
BUILD_TOP_DIR=/mnt/Data/Build/ffmpeg
...
cd rav1e-0.6.6
sudo cargo cinstall --prefix="$PREFIX_DIR" --libdir="$LIB_DIR" --library-type=staticlib --crt-static --release
sudo ldconfig
hash -r
}

# Had to replace the checkinstall line
CompileLibva(){
echo -e "\nBuilding & installing libva from repositories"
cd "$BUILD_SOURCE_DIR"
git clone https://github.com/intel/libva.git
cd libva
./autogen.sh --prefix="$PREFIX_DIR" --libdir="$LIB_DIR" --enable-x11=yes --enable-glx=yes
...
cd libva-utils
./autogen.sh --prefix="$PREFIX_DIR" --libdir="$LIB_DIR"
./configure
time make -j$(nproc) VERBOSE=1
...
mkdir -p aom_build
cd aom_build
PATH="$BIN_DIR:$PATH" cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="$PREFIX_DIR" -DCMAKE_INSTALL_LIBDIR="$LIB_DIR" -DENABLE_TESTS=OFF -DENABLE_NASM=on ../aom
...
cd SVT-AV1/Build
PATH="$BIN_DIR:$PATH" cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="$PREFIX_DIR" -DCMAKE_INSTALL_LIBDIR="$LIB_DIR" -DCMAKE_BUILD_TYPE=Release -DBUILD_DEC=OFF -DBUILD_SHARED_LIBS=OFF ..
PATH="$BIN_DIR:$PATH" make -j$(nproc)
sudo make -j$(nproc) install
sudo ldconfig
hash -r
...
mkdir -p dav1d/build
cd dav1d/build
meson setup -Denable_tools=false -Denable_tests=false --default-library=static .. --prefix "$PREFIX_DIR" --libdir="$LIB_DIR"
ninja
sudo ninja install
sudo ldconfig
hash -r
...
meson setup -Denable_tests=false -Denable_docs=false --buildtype=release --default-library=static .. --prefix "$PREFIX_DIR" --bindir="$BIN_DIR" --libdir="$LIB_DIR"
ninja
sudo ninja install
sudo ldconfig
hash -r
...
sudo mkdir -p "$INCLUDE_DIR/AMF"
sudo mv AMF-1.4.29/amf/public/include/* "$INCLUDE_DIR/AMF/"
sudo ldconfig
hash -r
}
...
# sudo checkinstall -y --deldoc=yes --pkgname=ffmpeg-git --pkgversion=6.0~ubuntu23.04
sudo make -j$(nproc) install
